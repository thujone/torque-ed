...
const GET_RESOURCE_COUNTS = gql`
  query GetResourceCounts {
    schoolSystemsCount
    schoolsCount
    coursesCount
    semestersCount
...
export default function GroupedDashboard() {
  ...
  const [showSeedStudentsDialog, setShowSeedStudentsDialog] = useState(false);
  ...
  const { data: countsData } = useQuery(GET_RESOURCE_COUNTS);
  ...
  const handleSeedStudentsClick = () => {
    setShowSeedStudentsDialog(true);
  };
  ...
  const handleSeedStudentsConfirm = async () => {
    setIsSeedingStudents(true);
    try {
      const response = await fetch('/api/seed-students', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      alert('Error seeding students: ' + (error instanceof Error ? error.message : 'Unknown error'));
    } finally {
      setIsSeedingStudents(false);
      setShowSeedStudentsDialog(false);
    }
  };

  // Create a mapping for count field names
  const countMapping = {
    'school-systems': 'schoolSystemsCount',
    'schools': 'schoolsCount',
    'courses': 'coursesCount',
    'semesters': 'semestersCount',
    'holidays': 'holidaysCount',
    'classes': 'classesCount',
    'class-sessions': 'classSessionsCount',
    'users': 'usersCount',
  ...
  const groups = [
    {
      ...
      description: 'Manage schools and academic periods',
      lists: [
    ...
    },
    {
      ...
      description: 'Manage courses, class sections and sessions',
      lists: [
    ...
    },
    {
      ...
      lists: [
        { key: 'users', label: 'Faculty and Staff', description: 'All faculty and staff (admins, instructors, TAs)' },
  ...
  return (
    <PageContainer header={<Heading type="h3">TorqueEd Dashboard</Heading>}>
      <div style={{ padding: '0.5rem' }}>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <span style={{ 
                        ...
                        {countsData?.[countMapping[list.key]] ?? '...'}
                      </span>
        ...
        {isSuperAdmin && (
          <div style={{
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              ...
              <button
                onClick={handleSeedStudentsClick}
                style={{
                  background: '#0ea5e9',
                  color: 'white',
                  border: 'none',
        ...
        {/* Seed Students Confirmation Dialog */}
        {showSeedStudentsDialog && (
          <div style={{
            position: 'fixed',
            top: 0,
            left: 0,
            ...
            <div style={{
              ...
              <h3 style={{ color: '#0ea5e9', marginBottom: '1rem' }}>
                📚 Confirm Student Data Import
              </h3>
              <p style={{ marginBottom: '1rem', fontSize: '0.875rem' }}>
                This will import student data from the CSV file located at:
                <br />
                <code style={{ background: '#f3f4f6', padding: '0.25rem', borderRadius: '4px' }}>
                  /data/sample-student-roster.csv
                </code>
              </p>
              <p style={{ marginBottom: '1rem', fontSize: '0.875rem' }}>
                Students will be associated with the default school system and school.
              </p>
              <p style={{ marginBottom: '1rem', fontSize: '0.875rem', color: '#0ea5e9', fontWeight: '500' }}>
                ✅ Safe Operation: This will not overwrite existing students.
              </p>

              <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                <button
                  onClick={() => setShowSeedStudentsDialog(false)}
                  ...
                  style={{
                    ...
                    padding: '0.5rem 1rem',
                    fontSize: '0.875rem',
                    cursor: isSeedingStudents ? 'not-allowed' : 'pointer',
                    opacity: isSeedingStudents ? 0.5 : 1
                  }}
                >
                  Cancel
                </button>
                <button
                  onClick={handleSeedStudentsConfirm}
                  disabled={isSeedingStudents}
                  style={{
                    background: '#0ea5e9',
                    color: 'white',
                ...
                >
                  {isSeedingStudents ? 'Importing...' : 'Yes, Import Students'}
                </button>
              </div>
            </div>
          </div>
